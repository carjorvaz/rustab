...
pub(crate) fn write_tsv_row<W, I>(writer: &mut W, fields: I) -> io::Result<()>
where
    W: Write,
    I: IntoIterator,
    I::Item: AsRef<str>,
{
    ...
    for field in fields {
        ...
        write_tsv_field(writer, field.as_ref())?;
    }

    writer.write_all(b"\n")
}

fn write_tsv_field<W: Write>(writer: &mut W, value: &str) -> io::Result<()> {
    let mut encoded = [0; 4];
...
mod tests {
    ...
    #[test]
    fn tsv_row_normalizes_every_field_to_one_line() {
        ...
        write_tsv_row(
            ...
            [
                "b.7\t9",
                "title\nwith\ttab",
                "https://example.test/\rpath\tquery",
            ],
        )
        .unwrap();
